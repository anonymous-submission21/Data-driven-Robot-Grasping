...
@gin.configurable
def episode_to_transitions_v1(episode_data, continuous=True):
  ...
  for t, transition in enumerate(episode_data):
    ...
    del debug
    features = {}
    obs_t = obs_t.flatten().tolist()
    obs_tp1 = obs_tp1.flatten().tolist()
    features['S/img'] = _float_feature(obs_t) 
    features['S_p1/img'] = _float_feature(obs_tp1)
    if continuous:
      if isinstance(action, np.ndarray):
        action = action.flatten().tolist()
      features['A'] = _float_feature(action)
