...
from dql_grasping import tf_util
...
def pointnet_basic(state,
  ...
  with tf.variable_scope(scope, reuse=tf.AUTO_REUSE, use_resource=True):
    net = tf_util.conv2d(net, 64, [1,3],
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='conv1', bn_decay=0.9) 
    logging.info('conv1 %s', net.get_shape())

    net = tf_util.conv2d(net, 64, [1,1],
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='conv2', bn_decay=bn_decay)
    logging.info('conv2 %s', net.get_shape())

    net = tf_util.conv2d(net, 64, [1,1],
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='conv3', bn_decay=bn_decay)
    logging.info('conv3 %s', net.get_shape())

    net = tf_util.conv2d(net, 128, [1,1],
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='conv4', bn_decay=bn_decay)
    logging.info('conv4 %s', net.get_shape())

    net = tf_util.conv2d(net, 1024, [1,1],
                        padding='VALID', stride=[1,1],
                        bn=True, is_training=is_training,
                        scope='conv5', bn_decay=bn_decay)
    logging.info('conv5 %s', net.get_shape())
...
def state_net(state,
  ...
  with tf.variable_scope(scope, reuse=tf.AUTO_REUSE, use_resource=True):
    with slim.arg_scope(tf_modules.argscope(is_training=is_training)):
      ...
      d1 = tf.shape(net)[0]
      d2 = tf.shape(context)[0]
      multi = d2/d1
      net = tf.tile(net, [tf.cast(multi, tf.int32), 1])
