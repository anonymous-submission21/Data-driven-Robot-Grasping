...
@gin.configurable
def parse_tfexample_state(example_proto,
                          dims=3,
                          action_size=None):
  ...
  features = {'S/img': tf.FixedLenFeature((dims,), tf.float32),
              'A': action_feature_spec,
              'R': tf.FixedLenFeature((), tf.float32),
              'S_p1/img': tf.FixedLenFeature((dims,), tf.float32),
              'done': tf.FixedLenFeature((), tf.int64),
              't': tf.FixedLenFeature((), tf.int64)}
  parsed_features = tf.parse_single_example(example_proto, features)
  # Decode the jpeg-encoded images into numeric tensors.
...
@gin.configurable
def parse_tfexample_pointnet(example_proto,
                                  dims=3,
                                  action_size=None):
  ...
  features = {'S/img': tf.FixedLenFeature((dims*3,), tf.float32),
              'A': action_feature_spec,
              'R': tf.FixedLenFeature((), tf.float32),
              'S_p1/img': tf.FixedLenFeature((dims*3,), tf.float32),
              'done': tf.FixedLenFeature((), tf.int64),
              't': tf.FixedLenFeature((), tf.int64)}
  parsed_features = tf.parse_single_example(example_proto, features)
  # Decode the jpeg-encoded images into numeric tensors.
